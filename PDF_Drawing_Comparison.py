# -*- coding: utf-8 -*-
"""
PDF_Drawing_Comparison.py: Compare two engineering drawings in PDF and
"anaglyph algorithm" is used to compare two PDF drawing files.
The files are first converted to PNG format, then the PNG files are processed. The output file is
a composite of the two input files with deleted pixels highlighted in red and new pixels highlighted in blue.
This script modified open source script 'diff-dwing-new.py' from (https://github.com/HyGear/diff-dwg)
"""
from tkinter import *
...
class DiffApp(Frame):
    def __init__(self, master):
        ...
        self.grid()
        self.master.title("PDF Drawing Comparison(HSS)_Ver.01")
        ...
        Radiobutton(self, text="PDF Drawing Comparison", variable=v, value="1", fg='black').grid(row=0, column=0,
                                                                                                    sticky='S')
        # Radiobutton(self, text="Batch Drawing Comparison", variable=v, value="2", fg='black').grid(row=1, column=0,
        #                                                                                            sticky='S')
        ...
        status_lower = Message(self, textvariable=v_status_f, bd=1, relief=SUNKEN, anchor=W, width=330,
                               font=('arial', 8)).grid(row=4, column=0, sticky='WE')
...
def pdf2png(pdf, temp, pageNos):     # pdf file 위치 & Temp Dir을 변수로
    ...
    png = temp + basefile[0] + str(pageNos+1) + ".png"
    png = str(png)
    pdf = str(pdf)
    # print(pdf)
    # print(png)
...
def alignimage(align1, align2):
    ...
    h1, w1, c = img1.shape
    gray1 = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY)     # 이미지 색상을 흑백으로 변환
    gray1 = numpy.float32(gray1) # cornerharris() 함수 image 인자 형태가 흑백스케일 & float32 형태여야 함.
    # harris corner detection 을 이용한 함수.
    # 인자 1: image(흑백, float32 type), 2: 이웃 픽셀의 범위, 3: Sobel 미분에 사용된 인자 값, 4: Harris corner 검출 수학식 R에서 k값
    dst1 = cv2.cornerHarris(gray1, 5, 3, 0.04)
    # threadhold(img, thredhold_value, value, flag)
    # img: Grayscale 이미지, thredhold_value: 픽셀 문턱값, value: 픽셀 문턱값보다 클 때 적용되는 최대값
...
def array_to_image(mode, size, a):
    return Image.frombytes(mode, size, a.reshape(len(a) * len(mode), 1).astype(numpy.uint8).tobytes()) # tostring() -> tobytes()
...
def process_images():
    ...
    if len(doc1) != len(doc2):
        print("The Nos of pages is Not matched")
    else:
        for i in range(len(doc1)):
            img1_file = pdf2png(filePath1, tempdir, i)
            img2_file = pdf2png(filePath2, tempdir, i)
            if check.get() == 1:
                align1, align2 = alignimage(img1_file, img2_file)
                im1, im2 = Image.open(align2), Image.open(align1)
            else:
                im1, im2 = Image.open(img2_file), Image.open(img1_file)
            file_string = os.path.splitext(os.path.basename(filePath1))[0] + "(" + str(i+1) + ")" + "_diff.png"
            if im1.size[0] == im2.size[0] and im1.size[1] == im2.size[1]:
                print("Drawing sizes match")
                if platform == "win32":
